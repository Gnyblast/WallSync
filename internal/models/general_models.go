...
type Args struct {
	...
	AIArtFilter       int    `json:"ai_art_filter" default:"1" help:"WallHaven: AI Art Filter"`
	Refresh           int    `json:"-" default:"10" help:"Refresh background every x Minutes. Default is 10"`
	Page              int    `json:"page" arg:"-"`
	OutputDir         string `json:"-" arg:"required" help:"Output directory to save image files: there will be a subdirectory created named 'WallSync'"`
	Command           string `json:"-" arg:"required" help:"Binary name to be used as background manager. i.e: feh"`
	ArgumentsTemplate string `json:"-" arg:"required" help:"Arguments for the command with %s used for image path substitution: i.e: --bg-fill %s"`
	MaxImages         int    `json:"-" arg:"required" help:"Max number of images that will be stored in disk"`
	ImageNamePrefix   string `json:"-" help:"Prefix for the image names that will be written to disk"`
